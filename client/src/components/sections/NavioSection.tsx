...
export const NavioSection: React.FC<Props> = ({}) => {
  ...
  const tabsUpdateOptionsBadge = () => navio.tabs.updateOptions("SettingsTab", { tabBarBadge: randomNum() });
  const tabsUpdateOptionsTitle = () => navio.tabs.updateOptions("SettingsTab", { title: `Random ${randomNum()}` });
  ...
  return (
    <Section title={t.do("section.navio.title")}>
      ...
      <BButton marginV-s1 size="small" label={t.do("section.navio.button.common.set_root")} onPress={globalSetRoot} />
      ...
      <BButton marginV-s1 size="small" label={t.do("section.navio.button.modals.show")} onPress={modalsShow} />
    </Section>
  );
};
