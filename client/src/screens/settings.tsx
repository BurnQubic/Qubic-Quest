...
export const Settings: NavioScreen = observer(({}) => {
  ...
  useEffect(() => {
    navigation.setOptions({
      headerRight: () => (unsavedChanges ? <HeaderButton onPress={handleSave} label="Save" /> : null),
    });
  }, [unsavedChanges, appearance, language]);

  // Methods
  const handleAppearanceIndexChange = (index: number) => setAppearance(appearanceUIToInternal[appearancesUI[index]]);
  const handleLanguageIndexChange = (index: number) => setLanguage(languageUIToInternal[languagesUI[index]]);
