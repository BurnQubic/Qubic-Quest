...
const checkChocolateMatch = (swappedItems: [number, number], items: readonly LevelItem[]): MatchDetail[] => {
  ...
  const validChocolateSwap =
    itemTypes.some((x) => x === "Chocolate") &&
    itemTypes.some((x) => validMatchItemTypes.includes(x)) &&
    firstItemType !== secondItemType;

  if (!validChocolateSwap) return [];

  const candyColor =
    (items[swappedItems[0]] as Candy).color || (items[swappedItems[1]] as Candy).color || DEFAULT_SWAPPED_CANDY_COLOR;
...
const getMatchGroups = (matchList: MatchDetail[], itemsList: readonly LevelItem[]): MatchGroup[] => {
  const matchedCandyList = matchList.filter(
    (x) => x.matched && CANDY_TYPES_ARRAY.includes(itemsList[x.index]?.type || "")
  );
...
export const checkForMatchings = (
  items: readonly LevelItem[],
...
export const getMatchGroupCenterIndex = (matchGroup: MatchGroup, matchList: MatchDetail[]): number => {
  ...
  const matchBalanceList: number[] = matchDetails.map((x) =>
    Math.abs(Math.abs(x.up - x.down) - Math.abs(x.left - x.right))
  );
...
export const levelHasPossibleCombinations = (
  itemList: readonly LevelItem[],
  tileList: readonly LevelTile[]
): boolean => {
