...
class LevelManager {
  ...
  subscribeItemsChange = (callback: (items: LevelItem[], matched: boolean) => void) =>
    this.itemsChangeSubscribers.push(callback);
  unsubscribeItemsChange = (callback: (items: LevelItem[], matched: boolean) => void) =>
    (this.itemsChangeSubscribers = this.itemsChangeSubscribers.filter((x) => x !== callback));
  notifyItemsChange = () =>
    this.itemsChangeSubscribers.forEach((callback) => callback(this._levelData.items, this._levelData.matched));

  subscribeItemsRerender = (callback: (items: LevelItem[]) => void) => this.itemsRerenderSubscribers.push(callback);
  unsubscribeItemsRerender = (callback: (items: LevelItem[]) => void) =>
  ...
  unsubscribeSequenceStart = (callback: () => void) =>
    (this.sequenceStartSubscribers = this.sequenceStartSubscribers.filter((x) => x !== callback));
  ...
  unsubscribeSequenceEnd = (callback: () => void) =>
    (this.sequenceEndStartSubscribers = this.sequenceEndStartSubscribers.filter((x) => x !== callback));
